...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      } else if (normalizedRole === 'recycling_centre' || normalizedRole === 'recycler') {
      ...
      let errorMessage = 'Connection failed. Please check your network or try again.';
      if (error.response?.data?.message) {
        errorMessage = error.response.data.message;
      }

      if (error.response?.data?.message) {
        // Backend now returns runtime exception message
        errorMessage = error.response.data.message;
        if (error.response.data.message.includes("pending approval")) {
          errorMessage = "Your account is awaiting Admin approval. You cannot login yet.";
        }
      }
