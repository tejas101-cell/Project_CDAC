                    ...
                    <div className="col-12 col-md-10 col-lg-8">
                        <div className="card border-0 shadow-lg rounded-4 overflow-hidden">
                            <div className="card-header bg-white border-0 py-4 px-4 px-md-5 d-flex justify-content-between align-items-center flex-wrap gap-3">
                                <div>
                                    <h5 className="text-muted text-uppercase small fw-bold mb-1">Request #{request.id}</h5>
                                    ...
                                    <div className="position-relative m-4">
                                        <div className="progress" style={{ height: '2px' }}>
                                            <div className="progress-bar bg-teal" role="progressbar"
                                                style={{ width: request.status === 'COMPLETED' ? '100%' : request.status === 'PICKED_UP' ? '75%' : request.status === 'SCHEDULED' || request.status === 'ACCEPTED' ? '50%' : '25%', backgroundColor: '#0f766e' }}></div>
                                        </div>
                                        ...
                                        <div className={`position-absolute top-0 start-50 translate-middle btn btn-sm rounded-pill ${['ACCEPTED', 'SCHEDULED', 'PICKED_UP', 'COMPLETED'].includes(request.status) ? 'btn-primary' : 'btn-secondary'}`} style={{ backgroundColor: ['ACCEPTED', 'SCHEDULED', 'PICKED_UP', 'COMPLETED'].includes(request.status) ? '#0f766e' : '#e2e8f0', border: 'none' }}>2</div>
                                        <div className={`position-absolute top-0 start-100 translate-middle btn btn-sm rounded-pill ${request.status === 'COMPLETED' ? 'btn-primary' : 'btn-secondary'}`} style={{ backgroundColor: request.status === 'COMPLETED' ? '#0f766e' : '#e2e8f0', border: 'none' }}>3</div>
                                    </div>
