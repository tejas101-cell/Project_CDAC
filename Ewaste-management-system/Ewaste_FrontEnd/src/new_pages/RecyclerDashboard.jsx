...
const RecyclerDashboard = () => {
    const [activeTab, setActiveTab] = useState('New Material');
    ...
    // Placeholder data representing materials arrived from collectors
    ...
    const filteredInventory = inventory.filter(item => {
        if (activeTab === 'New Material') return item.status === 'New Material';
        if (activeTab === 'Processing') return item.status === 'Processing';
        return item.status === 'Completed';
    });

    const stats = [
        { label: 'Unprocessed Items', count: inventory.filter(i => i.status === 'New Material').length, icon: 'bi-box-seam', color: '#0ea5e9', bg: '#e0f2fe' },
        { label: 'In Processing', count: inventory.filter(i => i.status === 'Processing').length, icon: 'bi-gear-wide-connected', color: '#f59e0b', bg: '#fef3c7' },
        { label: 'Total Recycled', count: inventory.filter(i => i.status === 'Completed').length, icon: 'bi-recycle', color: '#10b981', bg: '#d1fae5' }
    ];
            ...
            <main className="flex-grow-1 container py-5 mt-5">
                ...
                <div className="d-flex justify-content-between align-items-center mb-4 bg-white p-4 rounded shadow-sm border-start border-4" style={{ borderLeftColor: '#0ea5e9' }}>
                    ...
                    <button className="btn d-flex align-items-center px-4 py-2 text-white" style={{ backgroundColor: '#0ea5e9', border: 'none', borderRadius: '50px' }}>
                        ...
                        <span className="fw-semibold">Log Batch</span>
                    </button>
                </div>
                ...
                <div className="card border-0 shadow-sm rounded-3 overflow-hidden bg-white">
                    <div className="card-header bg-white border-bottom p-3 d-flex justify-content-between align-items-center">
                        <h5 className="fw-bold mb-0 text-secondary">Material Inventory</h5>
                        <div className="btn-group">
                            <button className={`btn btn-sm ${activeTab === 'New Material' ? 'btn-info text-white' : 'btn-outline-secondary'}`} onClick={() => setActiveTab('New Material')}>Arrivals</button>
                            ...
                            <button className={`btn btn-sm ${activeTab === 'History' ? 'btn-success text-white' : 'btn-outline-secondary'}`} onClick={() => setActiveTab('History')}>Processed</button>
                        </div>
                    </div>
                    <div className="table-responsive">
                        <table className="table table-hover align-middle mb-0">
                            <thead className="bg-light">
                                <tr>
                                    <th className="px-4 py-3 border-0 small text-uppercase text-muted fw-bold">Batch ID</th>
                                    <th className="py-3 border-0 small text-uppercase text-muted fw-bold">Source</th>
                                    <th className="py-3 border-0 small text-uppercase text-muted fw-bold">Material Type</th>
                                    <th className="py-3 border-0 small text-uppercase text-muted fw-bold">Weight</th>
                                    <th className="py-3 border-0 small text-uppercase text-muted fw-bold">Arrived On</th>
                            ...
                            <tbody>
                                {filteredInventory.length === 0 ? (
                                    <tr>
                                        <td colSpan="7" className="text-center py-5">
                                            <div className="d-flex flex-column align-items-center text-muted">
                                                ...
                                                <p className="mb-0">No materials currently in this stage.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    filteredInventory.map(item => (
                                        <tr key={item.id}>
                                            <td className="px-4 fw-medium">{item.id}</td>
                                            <td>{item.origin}</td>
                                            <td><span className="badge bg-light text-dark border">{item.type}</span></td>
                                            <td>{item.weight}</td>
                                            <td>{item.date}</td>
                                            <td>
                                                <span className={`badge ${item.status === 'New Material' ? 'bg-info text-white' :
