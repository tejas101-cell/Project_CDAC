...
const CreateRequest = () => {
    ...
    const handleAddItem = () => {
        ...
        if (!finalItemName || currentItem.quantity < 1) {
            showNotify("Please select an item name and quantity.", "danger");
            return;
        }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (items.length === 0) {
            showNotify("Please add at least one item to the request.", "danger");
            return;
        }
        ...
        try {
            // BACKEND: Only one call to PICKUP-SERVICE is needed.
            // PICKUP-SERVICE handles saving the request and notifying other services.
            const response = await requestService.createRequest(payload);
            const newRequestId = response.data.requestId;

            showNotify(`E-Waste request submitted successfully! Your Request ID is: #${newRequestId}`);
            // Redirect to dashboard
            navigate('/dashboard');
        } catch (error) {
            console.error("Error submitting request:", error);
            showNotify("Failed to submit request: " + (error.response?.data?.message || error.message), "danger");
        }
    };
