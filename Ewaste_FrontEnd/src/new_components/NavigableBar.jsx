          ...
          <ul className="navbar-nav ms-auto align-items-center">
              ...
              <>
                ...
                {((user.role && user.role.toLowerCase() === 'admin') || (user.roleName && user.roleName.toLowerCase() === 'admin')) && (
                  <>
                    <li className="nav-item px-2">
                      <Link className="nav-link text-white" to="/" onClick={closeNav}>Home</Link>
                    </li>
                    <li className="nav-item px-2">
                      <Link className="nav-link text-white" to="/admin/dashboard" onClick={closeNav}>Users</Link>
                    </li>
                    <li className="nav-item px-2">
                      <Link className="nav-link text-white" to="/admin/manage-requests" onClick={closeNav}>Approve Requests</Link>
                    </li>
                  </>
                )}
                ...
                {!((user.role && user.role.toLowerCase() === 'admin') || (user.roleName && user.roleName.toLowerCase() === 'admin')) && (user.role?.toLowerCase() === 'user' || user.roleName?.toLowerCase() === 'user' || (!user.role && !user.roleName && user.email)) && (
                  <>
                    ...
                    <li className="nav-item">
                      <Link className="nav-link text-white" to="/dashboard" onClick={closeNav}>Dashboard</Link>
                    </li>
                    {!isCreateRequestPage && !isViewRequestsPage && (
                      <li className="nav-item ms-lg-2">
                        <Link className="nav-action-btn" to="/create-request" onClick={closeNav}>
                    ...
                    {!isViewRequestsPage && (
                      <li className="nav-item ms-lg-2">
                        <Link className="nav-action-btn" to="/my-requests" onClick={closeNav}>
                ...
                {((user.role && user.role.toLowerCase() === 'collector') || (user.roleName && user.roleName.toLowerCase() === 'collector')) && (
                  <>
                    ...
                    <li className="nav-item px-2">
                      <Link className="nav-link text-white" to="/collector/dashboard" onClick={closeNav}>Dashboard</Link>
                    </li>
                  </>
                )}
                {((user.role && user.role.toLowerCase() === 'recycler') || (user.roleName && user.roleName.toLowerCase() === 'recycler')) && (
                  <>
                    ...
                    <li className="nav-item px-2">
                      <Link className="nav-link text-white" to="/recycler/dashboard" onClick={closeNav}>Dashboard</Link>
                    </li>
                  </>
                )}
            ...
            {!user && (
              <>
                ...
                <li className="nav-item px-2">
                  <Link className="nav-link text-white" to="/login" onClick={closeNav}>Login</Link>
                </li>
                <li className="nav-item ms-lg-3 w-100 w-lg-auto">
                  <Link className="btn btn-outline-light px-4 py-2 w-100 w-lg-auto" to="/register" onClick={closeNav} style={{ borderRadius: '4px' }}>
                    Get Started
                  </Link>
                </li>
              </>
